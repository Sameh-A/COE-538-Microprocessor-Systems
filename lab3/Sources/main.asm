...
lb1         MOVB #$90,ATDCTL5     ;r.just., unsign., sing.conv., mult., ch0, start conv.
            ...
            LDAA ATDDR4           ;load the ch4 result into AccA  (Form ATDDRy, y=0...7)
            LDAB #39              ;AccB = 39
            ...
            LDAA #$8F             ;move LCD cursor to the 1st row, end of msg1 (LCD cursor goes from $80 -> $8F for First row)
            JSR cmd2LCD           ;    "
            ...
            LDAA THOUSANDS        ;output the THOUSANDS ASCII charecter
            JSR putcLCD           ;    "
            
            LDAA #46              ;output the '.' charecter in terms of decimal value 46. 46 decimal value becomes '.' ASCII charecter 
            JSR putcLCD           ;    "
            
            LDAA HUNDREDS         ;output the HUNDREDS ASCII charecter.
            JSR putcLCD           ;    "
            
            LDAA #$CF             ;move LCD cursor to the 2nd row, end of msg2 (LCD cursor goes from $C0 -> $CF for Second row)
            JSR cmd2LCD           ;    "
...
bowOFF      JSR putcLCD
            LDAA #32              ;output a space character in ASCII  ( 32 in decimal becomes ' ' in ASCII)
            JSR putcLCD;
...
; Subroutine section
;*******************************************************************
;* Initialization of the LCD: 8-bit data width, 2-line display,    *
;* turn on display, cursor and blinking off. Shift cursor right.   *
;*******************************************************************
initLCD     BSET DDRE,%11111111 ; configure pins 7 -> 0 for output
            BSET DDRS,%11000000 ; configure pins E,RS
...
iloop:   PSHA ;2 E-clk           |
         PULA ;3 E-clk           |
         PSHA ;2 E-clk           | 50us(30 loops, 40 clocks,30x40 = 120 clocks, 120*41.67 = 50000ns = 50us)
         PULA ;3 E-clk           |
         NOP  ;1 E-clk           |
         NOP  ;1 E-clk           |
         NOP  ;1 E-clk           |
...
;********************************************
;*10. BCD to ASCII conversion Version  2    *                                      
;*                                          *
;********************************************
